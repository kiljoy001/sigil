...
int sigil_store_init(sigil_store_t *st, size_t capacity)
{
	...
	if (!st->lsh || !st->timestamp || !st->category || !st->trits || !st->hash) {
		sigil_store_free(st);
		return -1;
	}
...
static int store_grow(sigil_store_t *st)
{
	...
	if (!lsh || !ts || !cat || !tr || !hash) {
		free(lsh); free(ts); free(cat); free(tr); free(hash);
		return -1;
	}
	...
	free(st->lsh); free(st->timestamp); free(st->category);
	free(st->trits); free(st->hash);

	st->lsh = lsh; st->timestamp = ts; st->category = cat;
	st->trits = tr; st->hash = hash;
	st->capacity = cap;
	return 0;
}
